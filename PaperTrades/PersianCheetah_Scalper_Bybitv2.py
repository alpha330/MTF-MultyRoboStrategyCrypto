...
A simple scalping bot for Bybit using RSI, Bollinger Bands, and Volume indicators.
This bot is designed for paper trading and uses the Bybit API.

THIS IS V2 Add tuning Strateg on ETH 
And Set TimeOut for Open Position
'''
...
class PersianCheetahScalper:
    ...
    def get_positions_stats(self):
        ...
        total_pnl = sum(t['pnl'] for t in self.wallet.trade_history if t['symbol'] == self.symbol)
        return {
...
async def positions(update, context):
    ...
    for bot in manager.bots:
        ...
        stats += f"Total PnL: {pos_stats['total_pnl']:.2f} USDT\n\n"
    ...
    stats += f"Total PnL: {total_pnl:.2f} USDT\n"
