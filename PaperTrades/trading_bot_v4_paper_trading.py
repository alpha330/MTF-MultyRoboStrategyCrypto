...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/root/MTF-MultyRoboStrategyCrypto/initialBot/trading_3bot_ICHI_paper_mainnet.log'),
...
class TradingBot:
    def __init__(self, symbol, timeframe, indicators, higher_timeframe=None, higher_bot=None, leverage=5, risk_percent=0.01, initial_balance=100):
        ...
        self.exchange = ccxt.bybit({'enableRateLimit': True})  # No API key needed for mainnet OHLCV
    ...
    def fetch_ohlcv(self, timeframe):
        for _ in range(3):
            try:
                ...
                logger.info(f"{bcolors.OKGREEN}FOR OHLCV DATA {self.symbol} IN {timeframe} CANDLES ARE GET: {len(df)} (MAINNET)")
                if len(df) < 50:
...
async def start(update, context):
    ...
    for bot in manager.bots:
        bot.running = True
    await update.message.reply_text("All trading bots started (Paper Trading, Mainnet)")
    logger.info(f"{bcolors.OKGREEN}All bots started via Telegram (Paper Trading, Mainnet)")

async def stop(update, context):
    global manager
    for bot in manager.bots:
        bot.stop()
    await update.message.reply_text("All trading bots stopped (Paper Trading, Mainnet)")
    logger.info(f"{bcolors.WARNING}All bots stopped via Telegram (Paper Trading, Mainnet)")
