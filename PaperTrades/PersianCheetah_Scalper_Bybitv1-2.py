...
async def positions(update, context):
    ...
    for bot in manager.bots:
        ...
        stats += f"PersianCheetah_Scalper_Bybit V1.1 Stats for {bot.symbol}:\n"
...
async def main():
    ...
    try:
        manager = BotManager()
        trading_pairs = ['BTCUSDT', 'SOLUSDT', 'XRPUSDT']
        ...
        await telegram_bot.send_message(
            chat_id=TELEGRAM_CHAT_ID,
            text="PersianCheetah_Scalper_Bybit v1.1 initialized for BTC, SOL, XRP (1m, Paper Trading, Futures)"
        )
    except Exception as e:
