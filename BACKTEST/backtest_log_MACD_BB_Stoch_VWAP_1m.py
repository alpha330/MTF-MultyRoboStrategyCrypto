...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('backtest_log_MACD_BB_Stoch_VWAP_1m.log'),
...
class BacktestBot:
    def __init__(self, initial_balance=500, leverage=3, risk_percent=0.1, fee_rate=0.0006):
    ...
    def run_backtest(self):
        ...
        # اجرای بک‌تست
        min_data_length = 21  # حداقل تعداد کندل برای MACD
        for index, row in self.df_1m.iterrows():
            ...
            # فیلتر زمان برای جلوگیری از سیگنال‌های مکرر (هر 60 کندل)
            if self.last_signal_time and (index - self.last_signal_time) < 60 and position is None:
                continue
            ...
            if (not pd.isna(current_price) and not pd.isna(bb_middle) and not pd.isna(bb_lower) and 
                ...
                not pd.isna(volume_ma) and current_volume > 1.5 * volume_ma):
                if (macd > macd_signal and current_price > bb_middle + 0.4 * atr and 
                    stoch_rsi_k < 60 and current_price > vwap + 0.4 * atr):
                    signal = 'Short'
                    signal_count['Short'] += 1
                elif (macd < macd_signal and current_price < bb_middle - 0.4 * atr and 
                      stoch_rsi_k > 40 and current_price < vwap - 0.4 * atr):
                    signal = 'Long'
                    signal_count['Long'] += 1
            else:
                logger.debug(f"Missing data at index {index}: Price={current_price}, BB_Middle={bb_middle}, MACD={macd}, MACD_Signal={macd_signal}, StochRSI_K={stoch_rsi_k}, VWAP={vwap}, ATR={atr}, Volume_MA={volume_ma}")
            ...
            if position is None and signal != 'Neutral':
                ...
                position = signal
                stop_loss = entry_price - (3.5 * atr) if signal == 'Long' else entry_price + (3.5 * atr)
                ...
                # خروج از باند Bollinger
                exit_bb = False
                if position == 'Long' and current_price < bb_lower:
                    exit_bb = True
                    logger.info(f"Closing Long due to breaking below BB_Lower: {current_price}")
                elif position == 'Short' and current_price > bb_upper:
                    exit_bb = True
