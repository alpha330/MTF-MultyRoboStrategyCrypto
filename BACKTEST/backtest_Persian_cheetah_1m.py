...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('backtest_log_persian_cheetah_1m.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class BacktestPersianCheetah:
    def __init__(self, initial_balance=100, leverage=20, risk_percent=0.01, fee_rate=0.0002, symbol='BTCUSDT'):
    ...
    def check_tp_sl_trailing(self, current_price, bb_middle, index):
        for pos in self.positions[:]:
            stop_loss = pos['stop_loss']
            take_profit = pos['take_profit']
            side = pos['side']
            pos_time = pd.to_datetime(pos['timestamp'])
            current_time = self.df_1m['timestamp'].iloc[index]
            if (current_time - pos_time).total_seconds() > 1800:
    ...
    def run_backtest(self):
        logger.info("Starting backtest for PersianCheetah strategy on 1m timeframe...")
        ...
        for index, row in self.df_1m.iterrows():
            ...
            self.check_tp_sl_trailing(current_price, bb_middle, index)
            ...
            if long_position and rsi > rsi_short:
                self.close_position(long_position, current_price, reason=f'RSI above {rsi_short}')
                continue
            if short_position and rsi < rsi_long:
                self.close_position(short_position, current_price, reason=f'RSI below {rsi_long}')
                continue
            ...
            if signal and not long_position and not short_position:
                if self.balance <= 0:
                    ...
                    break
                stop_loss = current_price * (1 - 0.005) if signal == 'Long' else current_price * (1 + 0.005)
                take_profit = current_price * (1 + 0.01) if signal == 'Long' else current_price * (1 - 0.01)
...
if __name__ == "__main__":
    backtest = BacktestPersianCheetah(initial_balance=100, leverage=20, symbol='BTCUSDT')
