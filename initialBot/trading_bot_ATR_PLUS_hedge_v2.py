...
class TradingBot:
    def __init__(self, symbol, timeframe, indicators, higher_timeframe=None, higher_bot=None, leverage=5, risk_percent=0.01):
        ...
        self._set_leverage()  # Fixed: Changed from self.set_leverage() to self._set_leverage()
        ...
        try:
            ...
            for pos in position_info:
                if pos['symbol'] == self.symbol:
                    ...
                    break
            logger.info(f"CURRENT LEVERAGE FOR {self.symbol}: {current_leverage}")
    ...
    def fetch_ohlcv(self, timeframe):
        for _ in range(3):
            try:
                ohlcv = self.exchange.fetch_ohlcv(self.symbol, timeframe, limit=400)  # Increased to 400 candles
    ...
    def close_position(self, position):
        try:
            ...
            side = 'sell' if position['side'] == 'buy' else 'buy'  # In Hedged mode, side is explicitly buy/sell
            order = self.exchange.create_market_order(
                ...
                params={
                    'category': 'linear', 
                    'reduceOnly': True,
                    'positionIdx': 1 if position['side'] == 'buy' else 2  # 1 for Long, 2 for Short in Hedged mode
                }
            )
    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                # Check for Long and Short positions separately in Hedged mode
                ...
                # In Hedged mode, we can have both Long and Short positions, so no need to block opening a new position
                signal = None
                higher_signal = self.check_higher_timeframe()
                ...
                if 'RSI' in self.indicators and 'Stochastic' in self.indicators and 'Volume' in self.indicators:
                    ...
                    # Simplified conditions for signal
                ...
                if signal:
                    ...
                    try:
                        params = {
                            ...
                            'positionIdx': 1 if signal == 'Long' else 2  # 1 for Long, 2 for Short in Hedged mode
                        }
                        if signal == 'Long':
                        ...
                        sleep(15)  # Increased delay to 15 seconds
                        position = self.get_open_position(side='buy' if signal == 'Long' else 'sell')
                        if position:
                            logger.info(f"{bcolors.OKBLUE}Active Position: {position}")
                        else:
                            logger.warning(f"{bcolors.WARNING}Opened Position Not Found after 15 seconds, retrying...")
                            sleep(10)  # Retry after 10 more seconds
                            position = self.get_open_position(side='buy' if signal == 'Long' else 'sell')
                            if position:
                                logger.info(f"{bcolors.OKBLUE}Active Position Found on Retry: {position}")
                            else:
